...
export const  ChangeProFileData = () => {
    ...
    const handleClickChangeProfile = () => {
        if(user !== null) {
            dispatch(changeProfile(user.id, city, dataendpoint, name ))
        }
    }
    ...
    return (
        <div>
            {
            user !== null  && user.name === "" ? <div>Заполните все поля  <button onClick={handleEditProfile}>Редактировать Профиль</button> </div> : 
            user == null ? dispatch(push('/auth')) :  
                <div>
