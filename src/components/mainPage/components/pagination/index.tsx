...
export const PaginationButton = () => {
    ...
    useEffect(() => {
        todoInfo.length < 5 ? setDisableArrowRigth(true) : setDisableArrowRigth(false)
    ...
    return (<div>
      <button onClick ={() => dispatch(setCurrentPage(currentPage - 1))} disabled={disableArrowLeft}> <ArrowBackIosIcon/> </button> 
         <span className="number_current_page">{currentPage}</span> 
      <button onClick ={() => dispatch(setCurrentPage(currentPage + 1))} disabled = {disableArrowRigth}> <ArrowForwardIosIcon/> </button>
    </div>)
}